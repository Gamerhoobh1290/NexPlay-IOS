...
export class CommandPalette {
    constructor() {
        ...
        this.input.addEventListener('keydown', (/** @type {any} */ event) => {
        ...
        window.addEventListener('keydown', (/** @type {any} */ event) => {
            ...
            if (!this.isOpen && !inEditable) {
                ...
                if (!commandId) return;
                const command = this.commands.find((/** @type {any} */ entry) => entry.id === commandId);
                if (!command) return;
                event.preventDefault();
     ...
     */
    register(command) {
        const index = this.commands.findIndex((/** @type {any} */ item) => item.id === command.id);
    ...
    /** @type {(...args: any[]) => any} */
    listMacros() {
        return Object.values(this.macros).sort((/** @type {any} */ a, /** @type {any} */ b) => (b.updatedAt || 0) - (a.updatedAt || 0));
    }
    ...
    render() {
        ...
        this.list.innerHTML = visible.map((/** @type {any} */ command, /** @type {any} */ index) => {
        ...
        this.list.querySelectorAll('button[data-command-id]').forEach((/** @type {any} */ button) => {
            button.addEventListener('click', () => {
                const command = visible.find((/** @type {any} */ item) => item.id === button.getAttribute('data-command-id'));
    ...
    #filteredCommands() {
        ...
        return this.commands.filter((/** @type {any} */ command) => {
    ...
    #normalizeKeyCombo(input) {
        return String(input || '')
            .split('+')
            .map((/** @type {any} */ part) => part.trim().toLowerCase())
            .filter(Boolean)
            .join('+');
    }
