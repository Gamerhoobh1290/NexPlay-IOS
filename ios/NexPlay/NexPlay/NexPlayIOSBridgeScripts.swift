...
enum NexPlayIOSBridgeScripts {
    static let installPlaybackBridge: String = """
        (function installNexPlayIOSBridge() {
          if (window.__nexplayIOSBridgeInstalled) return;
          window.__nexplayIOSBridgeInstalled = true;

          var handler = window.webkit &&
            window.webkit.messageHandlers &&
            window.webkit.messageHandlers.nexplayIOS;
          ...
          function emit(reason) {
            ...
            post('playbackSnapshot', { snapshot: snapshot });
          }

          function wrapFunction(name, reason) {
            try {
              var fn = window[name];
              if (typeof fn !== 'function' || fn.__nexplayIOSWrapped) return;
              var wrapped = function() {
                var result = fn.apply(this, arguments);
                emit(reason || name);
                return result;
              };
              wrapped.__nexplayIOSWrapped = true;
              window[name] = wrapped;
            } catch (_) {}
          }

          window.__nexplayIOSCommands = {
            play: function() {
              try {
                var stateRef = getStateSafe();
            ...
            toggle: function() {
              try {
                var stateRef = getStateSafe();
                if (stateRef.isPlaying || stateRef.currentPlaybackSource === 'online-music') {
                  this.pause();
                } else {
                  this.play();
                }
          ...
          if (audioRef && !audioRef.__nexplayIOSBridgeEvents) {
            audioRef.__nexplayIOSBridgeEvents = true;
    ...
    static let playCommand: String = """
        (function() {
          var commands = window.__nexplayIOSCommands;
          if (commands && typeof commands.play === 'function') commands.play();
        })();
    """

    static let pauseCommand: String = """
        (function() {
          var commands = window.__nexplayIOSCommands;
    ...
    static let toggleCommand: String = """
        (function() {
          var commands = window.__nexplayIOSCommands;
          if (commands && typeof commands.toggle === 'function') commands.toggle();
        })();
    """

    static let nextCommand: String = """
        (function() {
          var commands = window.__nexplayIOSCommands;
    ...
    static let previousCommand: String = """
        (function() {
          var commands = window.__nexplayIOSCommands;
    ...
    static func seekToCommand(positionMs: Int) -> String {
        let clampedMs = max(0, positionMs)
        return """
            (function() {
              var commands = window.__nexplayIOSCommands;
              if (commands && typeof commands.seekToMs === 'function') {
                commands.seekToMs(\(clampedMs));
              }
            })();
        """
    }
