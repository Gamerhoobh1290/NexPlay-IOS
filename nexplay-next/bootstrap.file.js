(function () {
    ...
    bootstrap().catch(function (/** @type {any} */ error) {
    ...
    async function bootstrap() {
        ...
        var rulesPanel = createRulesPanel({
            ...
            saveRules: function (/** @type {any} */ rules) {
        ...
        window.NexPlayNext = {
            ...
            storage: {
                ...
                upsertAutomationRules: async function (/** @type {any} */ rules) {
                    saveRules(rules);
                }
            },
            sync: {
    ...
    function wireAutomationEvents(engine) {
        window.addEventListener('nexplay:app-ready', function (/** @type {any} */ event) {
            engine.evaluate('on_app_start', event.detail || {}).catch(function (/** @type {any} */ error) {
                console.warn('[NexPlayNext:file] on_app_start failed', error);
            });
        });
        window.addEventListener('nexplay:track-ended', function (/** @type {any} */ event) {
            engine.evaluate('on_track_end', event.detail || {}).catch(function (/** @type {any} */ error) {
                console.warn('[NexPlayNext:file] on_track_end failed', error);
            });
        });
        window.addEventListener('nexplay:import-complete', function (/** @type {any} */ event) {
            engine.evaluate('on_import_complete', event.detail || {}).catch(function (/** @type {any} */ error) {
                console.warn('[NexPlayNext:file] on_import_complete failed', error);
            });
        });
        /** @type {(...args: any[]) => any} */
        setInterval(function () {
            engine.evaluate('scheduled_time', { now: Date.now() }).catch(function (/** @type {any} */ error) {
    ...
    function resolveFeatureFlags() {
        ...
        /** @type {Record<string, any>} */
        var normalized = {};
        Object.keys(merged).forEach(function (/** @type {any} */ key) {
    ...
    /** @type {(...args: any[]) => any} */
    AutomationEngine.prototype.registerRule = function (/** @type {any} */ rule) {
        var cleaned = normalizeRule(rule, rule.priority);
        var index = this.rules.findIndex(function (/** @type {any} */ item) { return item.id === cleaned.id; });
        if (index === -1) this.rules.push(cleaned);
        else this.rules[index] = cleaned;
        this.rules.sort(function (/** @type {any} */ a, /** @type {any} */ b) { return (a.priority || 0) - (b.priority || 0); });
    };

    /** @type {(...args: any[]) => any} */
    AutomationEngine.prototype.setRules = function (/** @type {any} */ rules) {
        var self = this;
        this.rules = [];
        (Array.isArray(rules) ? rules : []).forEach(function (/** @type {any} */ rule, /** @type {any} */ index) {
    ...
    /** @type {(...args: any[]) => any} */
    AutomationEngine.prototype.registerActionHandler = function (/** @type {any} */ actionType, /** @type {any} */ handler) {
        this.actionHandlers.set(actionType, handler);
    };

    /** @type {(...args: any[]) => Promise<any>} */
    AutomationEngine.prototype.evaluate = async function (/** @type {any} */ triggerType, /** @type {any} */ payload) {
        var candidates = this.rules
            .filter(function (/** @type {any} */ rule) { return rule.enabled !== false; })
            .filter(function (/** @type {any} */ rule) { return rule.trigger && rule.trigger.type === triggerType; })
            .sort(function (/** @type {any} */ a, /** @type {any} */ b) { return (a.priority || 0) - (b.priority || 0); });
    ...
    /** @type {(...args: any[]) => Promise<any>} */
    AutomationEngine.prototype.runAction = async function (/** @type {any} */ action) {
    ...
    function registerDefaultAutomationHandlers(engine, legacy) {
        engine.registerActionHandler('set_speed', async function (/** @type {any} */ payload) {
            var speed = Number(payload.speed || 1);
            legacy.dispatchAction('set_speed', { speed: speed });
        });

        engine.registerActionHandler('toggle_mode', async function (/** @type {any} */ payload) {
            var mode = payload.mode || 'windowed';
            legacy.dispatchAction('toggle_mode', { mode: mode });
        });

        engine.registerActionHandler('start_playlist', async function (/** @type {any} */ payload) {
            ...
            var playlist = (state.playlists || []).find(function (/** @type {any} */ item) { return item.id === playlistId; });
        ...
        engine.registerActionHandler('apply_tags', async function (/** @type {any} */ payload) {
            var tags = Array.isArray(payload.tags) ? payload.tags.filter(Boolean) : [];
            if (!tags.length) return;
            var state = legacy.getState();
            var currentTrack = (state.tracks || []).find(function (/** @type {any} */ item) { return item.id === state.currentTrackId; });
        ...
        engine.registerActionHandler('enqueue_filter', async function (/** @type {any} */ payload) {
            var query = String(payload.query || '').toLowerCase();
            if (!query) return;
            var state = legacy.getState();
            var matching = (state.tracks || []).filter(function (/** @type {any} */ track) {
            ...
            matching.forEach(function (/** @type {any} */ track) {
    ...
    /** @type {(...args: any[]) => any} */
    ActionRegistry.prototype.register = function (/** @type {any} */ action, /** @type {any} */ handler) {
    ...
    ActionRegistry.prototype.bind = function () {
        var self = this;
        this.bound = true;
        this.root.addEventListener('click', function (/** @type {any} */ event) {
    ...
    function wireHeaderActions(commandPalette, automation) {
        var registry = /** @type {any} */ (new (/** @type {any} */ (ActionRegistry))(document));
    ...
    function installDevToolsConsoleGate(actionsWrap) {
        ...
        window.NexPlayOps = {
            unlock: function (/** @type {any} */ secret) {
    ...
    function CommandPalette() {
        ...
        this.input.addEventListener('keydown', function (/** @type {any} */ event) {
        ...
        window.addEventListener('keydown', function (/** @type {any} */ event) {
    ...
    /** @type {(...args: any[]) => any} */
    CommandPalette.prototype.register = function (/** @type {any} */ command) {
        var index = this.commands.findIndex(function (/** @type {any} */ item) {
    ...
    CommandPalette.prototype.filteredCommands = function () {
        ...
        return self.commands.filter(function (/** @type {any} */ command) {
    ...
    CommandPalette.prototype.render = function () {
        ...
        this.list.innerHTML = visible.map(function (/** @type {any} */ command, /** @type {any} */ index) {
        ...
        this.list.querySelectorAll('button[data-command-id]').forEach(function (/** @type {any} */ button) {
            button.addEventListener('click', function () {
                var id = button.getAttribute('data-command-id');
                var command = visible.find(function (/** @type {any} */ item) { return item.id === id; });
                if (command) self.run(command);
            });
        });
    };
    ...
    /** @type {(...args: any[]) => Promise<any>} */
    CommandPalette.prototype.run = async function (/** @type {any} */ command) {
        await command.run();
        this.close();
    };
    ...
    function createRulesPanel(options) {
        ...
        triggerInput.innerHTML = TRIGGERS.map(function (/** @type {any} */ v) { return '<option value="' + v + '">' + v + '</option>'; }).join('');
        actionInput.innerHTML = ACTIONS.map(function (/** @type {any} */ v) { return '<option value="' + v + '">' + v + '</option>'; }).join('');

        /** @type {(...args: any[]) => any} */
        function reindex() {
            state.rules = state.rules.map(function (/** @type {any} */ rule, /** @type {any} */ index) {
        ...
        function renderList() {
            ...
            listEl.innerHTML = state.rules.map(function (/** @type {any} */ rule, /** @type {any} */ idx) {
        ...
        /** @type {(...args: any[]) => any} */
        function open() {
            state.rules = options.getRules().map(function (/** @type {any} */ rule, /** @type {any} */ index) {
        ...
        state.root.addEventListener('click', function (/** @type {any} */ event) {
            if (event.target === state.root) close();
        });
        state.root.addEventListener('keydown', function (/** @type {any} */ event) {
            if (event.key === 'Escape') {
                event.preventDefault();
                close();
            }
        ...
        state.root.querySelector('#automation-add').addEventListener('click', function () {
            ...
            var index = state.rules.findIndex(function (/** @type {any} */ item) { return item.id === rule.id; });
        ...
        listEl.addEventListener('click', function (/** @type {any} */ event) {
            ...
            if (!action || !id) return;
            if (action === 'edit') {
                var rule = state.rules.find(function (/** @type {any} */ item) { return item.id === id; });
                if (!rule) return;
                ...
                state.rules = state.rules.filter(function (/** @type {any} */ item) { return item.id !== id; });
                reindex();
                if (state.editId === id) resetForm();
                renderList();
            } else if (action === 'move-up' || action === 'move-down') {
                var currentIndex = state.rules.findIndex(function (/** @type {any} */ item) { return item.id === id; });
        ...
        listEl.addEventListener('change', function (/** @type {any} */ event) {
            var target = event.target;
