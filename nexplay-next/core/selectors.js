...
export function selectTracks(state, filter = {}) {
    ...
    if (filter.media === 'audio') tracks = tracks.filter((/** @type {any} */ track) => track.type === 'audio');
    if (filter.media === 'video') tracks = tracks.filter((/** @type {any} */ track) => track.type === 'video');
    if (filter.media === 'favorites') tracks = tracks.filter((/** @type {any} */ track) => Boolean(track.isFavorite));
    if (filter.tag) tracks = tracks.filter((/** @type {any} */ track) => Array.isArray(track.tags) && track.tags.includes(filter.tag || ''));

    if (query) {
        tracks = tracks.filter((/** @type {any} */ track) => {
    ...
    tracks.sort((/** @type {any} */ left, /** @type {any} */ right) => {
        let value = 0;
...
export function selectCurrentTrack(state) {
    ...
    return state.tracks.find((/** @type {any} */ track) => track.id === state.playback.currentTrackId) || null;
}
...
export function selectQueuePreview(state) {
    ...
    return state.tracks.find((/** @type {any} */ track) => track.id === first) || null;
}
