...
export class PlaybackEngine {
    ...
    getNextTrackId(tracks, currentTrackId, queueState) {
        ...
        if (queueState.isShuffle) {
            const pool = queueState.shuffleQueue && queueState.shuffleQueue.length ? queueState.shuffleQueue : tracks.map((/** @type {any} */ track) => track.id);
        ...
        const index = tracks.findIndex((/** @type {any} */ track) => track.id === currentTrackId);
    ...
    preloadNextTrack(url) {
        ...
        const probe = /** @type {HTMLMediaElement} */ (document.createElement(this.media.tagName.toLowerCase()));
        probe.preload = 'auto';
        probe.src = url;
        this.preloadedNext = {
            url,
    ...
    getWaveformSnapshot(size = 96) {
        ...
        return Array.from(data).map((/** @type {any} */ value) => (value - 128) / 128);
    }
