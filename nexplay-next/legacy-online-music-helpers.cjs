(function attachOnlineMusicHelpers(/** @type {any} */ globalScope, /** @type {any} */ factory) {
    ...
    /** @type {(...args: any[]) => any} */
    function normalizeOnlineMusicSearchIntentPairs(values) {
        const seen = new Set();
        return (Array.isArray(values) ? values : [])
            .map((pair) => ({
                title: normalizeOnlineMusicTrackText(pair && pair.title ? pair.title : ''),
                artist: normalizeOnlineMusicTrackText(pair && pair.artist ? pair.artist : '')
            }))
            .filter((pair) => {
                if (!pair.title || !pair.artist) return false;
    ...
    function parseOnlineMusicSearchIntent(query) {
        ...
        if (byMatch) {
            ...
            structuredPairs.push({ title: byMatch[1], artist: byMatch[2] });
        }
        ...
        if (!byMatch && !dashMatch && normalized) {
            ...
            const tokens = splitOnlineMusicSearchTokens(normalized);
            // Two-word searches are commonly complete song or artist names
            // (for example, "Dirty Diana" or "Daft Punk"). Splitting them
            // into one-word titles made unrelated single-token matches look
            // exact. Keep the complete phrase and only infer artist/title
            // boundaries once there are enough tokens to form useful pairs.
            const maxSplit = tokens.length >= 3 ? Math.min(2, tokens.length - 1) : 0;
            for (let split = 1; split <= maxSplit; split += 1) {
                const leftTitle = tokens.slice(0, -split).join(' ');
                const rightTitle = tokens.slice(split).join(' ');
                const rightArtist = tokens.slice(-split).join(' ');
                const leftArtist = tokens.slice(0, split).join(' ');
                if (shouldUseAutoSplitTitleCandidate(leftTitle)) {
                    titleCandidates.push(leftTitle);
                    artistCandidates.push(rightArtist);
                    inferredPairs.push({ title: leftTitle, artist: rightArtist });
                }
                if (shouldUseAutoSplitTitleCandidate(rightTitle)) {
                    titleCandidates.push(rightTitle);
                    artistCandidates.push(leftArtist);
                    inferredPairs.push({ title: rightTitle, artist: leftArtist });
                }
            }
        }
        ...
        const normalizedStructuredPairs = normalizeOnlineMusicSearchIntentPairs(structuredPairs);
        const normalizedInferredPairs = normalizeOnlineMusicSearchIntentPairs(inferredPairs);
        ...
        return {
            ...
            structuredPairs: normalizedStructuredPairs,
            inferredPairs: normalizedInferredPairs,
            hasExplicitArtist: normalizedStructuredPairs.length > 0 || normalizedInferredPairs.length > 0,
            hasStructuredArtist: normalizedStructuredPairs.length > 0,
    ...
    /** @type {(...args: any[]) => any} */
    function scoreOnlineMusicSearchIntentPairs(pairs, titleTargets, artistTargets) {
        return (Array.isArray(pairs) ? pairs : []).reduce((best, pair) => {
            const titleScore = getMaxOnlineMusicTextMatchScore([pair && pair.title], titleTargets);
            const artistScore = getMaxOnlineMusicTextMatchScore([pair && pair.artist], artistTargets);
            const coherentScore = Math.min(titleScore, artistScore);
            if (coherentScore < best.coherentScore) return best;
            if (coherentScore === best.coherentScore && (titleScore + artistScore) <= (best.titleScore + best.artistScore)) {
                return best;
            }
            return { coherentScore, titleScore, artistScore };
        }, { coherentScore: 0, titleScore: 0, artistScore: 0 });
    }
    ...
    /** @type {(...args: any[]) => any} */
    function hasOnlineMusicAudioOrLyricsPlaybackMarker(value) {
        return /\b(?:official\s+audio|audio\s+only|lyric\s+video|lyrics?|visuali[sz]er|provided\s+to\s+youtube)\b/i.test(String(value || ''));
    }

    /** @type {(...args: any[]) => any} */
    function hasOnlineMusicMusicVideoMarker(value) {
        const raw = String(value || '');
        if (!raw || hasOnlineMusicAudioOrLyricsPlaybackMarker(raw)) return false;
    ...
    function hasOnlineMusicOfficialUploadMarker(value) {
        return /\b(?:official\s+audio|official\s+visuali[sz]er|official\s+lyric\s+video)\b/i.test(String(value || ''));
    }

    /** @type {(...args: any[]) => any} */
    function getOnlineMusicSearchResultModifierKinds(value) {
        const raw = String(value || '');
        const normalized = normalizeOnlineMusicReleaseText(raw);
        if (!normalized) return [];
        const modifierPatterns = [
            ['cover', /(?:[([{][^)\]}]{0,80}\bcover\b[^)\]}]{0,80}[)\]}]|\b(?:acoustic|piano|rock|metal|vocal|guitar)\s+cover\b|\bcover\s+(?:version|song|of|by)\b|\b(?:in\s+the\s+style\s+of|as\s+made\s+famous\s+by)\b)/i],
            ['karaoke', /\bkaraoke\b/i],
            ['reaction', /\b(?:reaction|reacts?)\b/i],
            ['preview', /\b(?:teaser|trailer|preview|snippet)\b/i],
            ['social-clip', /\b(?:challenge|meme|status|tiktok|shorts?|clips?|fanmade|fan\s+made|tribute|lesson|tutorial)\b/i],
            ['instrumental', /\binstrumental\b/i],
            ['acapella', /\b(?:a\s+cappella|acapella)\b/i],
            ['sped-up', /\bsped\s+up\b/i],
            ['slowed', /\b(?:slowed(?:\s+down)?|reverb)\b/i],
            ['nightcore', /\b(?:nightcore|8d)\b/i],
            ['mashup', /\bmashup\b/i],
            ['remix', /\b(?:remix|bootleg)\b/i],
            ['unreleased', /\b(?:leak|unreleased)\b/i],
            ['live', /(?:[([{]\s*(?:official\s+)?live(?:\s+(?:at|from|in|on)\b)?[^)\]}]*[)\]}]|\blive\s+(?:at|from|in|on|version|recording|performance)\b|\b(?:recorded|performed)\s+live\b|\b(?:concert|festival)\b)/i],
            ['long-form', /\b(?:full\s+album|playlist|compilation)\b/i],
            ['loop', /\b(?:loop(?:ed)?|extended\s+version)\b/i]
        ];
        return modifierPatterns
            .filter((entry) => entry[1].test(raw) || entry[1].test(normalized))
            .map((entry) => entry[0]);
    }

    /** @type {(...args: any[]) => any} */
    function hasDisallowedOnlineMusicSearchResultModifier(value, query) {
        const candidateKinds = getOnlineMusicSearchResultModifierKinds(value);
        if (!candidateKinds.length) return false;
        const requestedKinds = new Set(getOnlineMusicSearchResultModifierKinds(query));
        const normalizedQuery = normalizeOnlineMusicReleaseText(query);
        // In a query, a bare modifier is intentional ("Dirty Diana live").
        // Candidate titles use the stricter contextual patterns above so a
        // canonical title such as "Live Forever" is not treated as a live cut.
        if (/\blive\b/.test(normalizedQuery)) requestedKinds.add('live');
        if (/\bcover\b/.test(normalizedQuery)) requestedKinds.add('cover');
        return candidateKinds.some((kind) => !requestedKinds.has(kind));
    }
    ...
    function classifyOnlineMusicSearchResultEligibility(options) {
        ...
        const isYouTubeMusicSource = /\byoutube-music\b|ytmsearch/i.test(String([
            opts.sourceSurface,
            opts.resolver,
            opts.catalogProviderLabel,
            opts.providerLabel
        ].filter(Boolean).join(' ')));
        const intent = parseOnlineMusicSearchIntent(opts.query || '');
        ...
        if (hasDisallowedOnlineMusicSearchResultModifier(modifierText, intent.raw)) {
        ...
        const coherentArtistTargets = uniqueNormalizedOnlineMusicTexts([
            rawLyricsArtist,
            rawArtist,
            rawChannel,
            titleArtist,
        ...
        if (intent.hasStructuredArtist) {
            const explicitIdentity = scoreOnlineMusicSearchIntentPairs(
                intent.structuredPairs,
                titleTargets,
                coherentArtistTargets
            );
            if (explicitIdentity.titleScore < 112) {
                return { include: false, kind: '', score: -820, reason: 'explicit-title-mismatch' };
            }
            if (explicitIdentity.artistScore < 112) {
                return { include: false, kind: '', score: -840, reason: 'explicit-artist-mismatch' };
            }
        }
        ...
        if (hasOnlineMusicMusicVideoMarker(modifierText)) {
            return { include: false, kind: '', score: -880, reason: 'music-video' };
        }
        ...
        const queryArtistScore = channelArtist
            ? getMaxOnlineMusicTextMatchScore([intent.normalized], [channelArtist])
            : 0;
        ...
        const artistOnlyQueryMatchesOfficialSource = !intent.hasStructuredArtist
            && intent.tokenCount <= 3
            && channelArtist
            && queryArtistScore >= 112
            && (hasTopicChannel || hasVevoChannel || hasOfficialArtistChannel || hasProvidedToYouTube);
        const youtubeMusicSourceMatchesQuery = isYouTubeMusicSource
            && (strongTitleMatch || queryArtistScore >= 112 || channelMatchesDeclaredArtist || channelMatchesQueryArtist);

        if (!strongTitleMatch && artistOnlyQueryMatchesOfficialSource) {
            return {
                include: true,
                kind: hasTopicChannel ? 'topic-artist-result' : (hasVevoChannel ? 'vevo-artist-result' : 'official-artist-result'),
                score: 160 + queryArtistScore,
                reason: 'official-artist-search'
            };
        }

        if (!strongTitleMatch && youtubeMusicSourceMatchesQuery) {
            return {
                include: true,
                kind: 'youtube-music-result',
                score: 150 + queryArtistScore,
                reason: 'youtube-music-source'
            };
        }
        ...
        if (youtubeMusicSourceMatchesQuery) {
            return {
                include: true,
                kind: 'youtube-music-result',
                score: 210 + titleScore + Math.max(0, queryArtistScore),
                reason: 'youtube-music-source'
            };
        }
    ...
    function scoreOnlineMusicSearchResultForQuery(options) {
        ...
        const artistTargets = uniqueNormalizedOnlineMusicTexts([
            ...
            rawArtist,
            rawChannel,
            getOnlineMusicSplitArtistFromTitle(rawTitle),
            getOnlineMusicSplitArtistFromTitle(rawLyricsTitle)
        ]);
        if (intent.hasStructuredArtist) {
            const explicitIdentity = scoreOnlineMusicSearchIntentPairs(
                intent.structuredPairs,
            ...
            if (explicitIdentity.coherentScore < 112) return 0;
        }
        ...
        if (missingQuerySignificantTokens.length) {
            ...
            if (intent.tokenCount <= 2) {
                score = Math.min(score, 24);
            } else if (titleScore < 112 || intent.tokenCount >= 4 || missingQuerySignificantTokens.length >= 2) {
    ...
    function scoreOnlineMusicTrackCandidate(options) {
        ...
        const rawCandidateText = [
            opts.candidateTitle,
            opts.candidateArtist,
            opts.candidateChannel
        ].filter(Boolean).join(' ');
        ...
        if (hasOnlineMusicAudioOrLyricsPlaybackMarker(rawCandidateText)) score += 24;
        if (hasOnlineMusicMusicVideoMarker(rawCandidateText)) score -= 160;
        return Math.max(0, score);
    }
