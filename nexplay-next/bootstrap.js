...
function patchLegacyIntegrations({ legacy, jobs, sync, observability, enrichmentWorker }) {
    ...
    if (typeof window.fetchLyrics === 'function') {
        ...
        window.fetchLyrics = async (...args) => jobs.add(async () => {
            ...
            const workerLyricsPromise = enrichmentWorker && artist && title
                ? enrichmentWorker.lyricsLookup(artist, title).catch(() => null)
                : null;
            const result = await originalFetchLyrics(...args);

            if (workerLyricsPromise && track && !track.assignedLyricsRaw) {
                const workerLyrics = await workerLyricsPromise;
                if (workerLyrics && workerLyrics.kind === 'synced' && workerLyrics.raw && !track.assignedLyricsRaw) {
                    track.assignedLyricsRaw = workerLyrics.raw;
                    track.assignedLyricsSource = 'Synced';
