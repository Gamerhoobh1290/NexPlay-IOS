...
export class RulesPanel {
    ...
    constructor(options) {
        ...
        this.triggerInput.innerHTML = TRIGGERS.map((value) => `<option value="${value}">${value}</option>`).join('');
        this.actionInput.innerHTML = ACTIONS.map((value) => `<option value="${value}">${value}</option>`).join('');
    ...
    open() {
        this.buffer = this.getRules().map((rule) => ({ ...rule, actions: (rule.actions || []).slice() }));
        this.render();
    ...
    render() {
        ...
        this.listEl.innerHTML = this.buffer.map((rule) => `
            <div class="rounded border border-white/10 bg-white/5 p-3">
                <div class="flex items-center justify-between gap-2">
                    <div>
                        <div class="text-sm font-semibold">${escapeHtml(rule.name)}</div>
                        <div class="text-[11px] text-gray-400">${rule.trigger.type} -> ${(rule.actions || []).map((action) => action.type).join(', ')}</div>
                    </div>
        ...
        this.listEl.querySelectorAll('input[data-toggle]').forEach((input) => {
            input.addEventListener('change', () => {
                const id = input.getAttribute('data-toggle');
                const rule = this.buffer.find((item) => item.id === id);
    ...
    async #save() {
        await this.onSave(this.buffer.map((rule, index) => ({ ...rule, priority: index + 1, updatedAt: Date.now() })));
        this.close();
    }
}
