...
export function createStore(initialState, reducer) {
    ...
    return {
        ...
        replaceState(nextState) {
            ...
            listeners.forEach((listener) => listener(state, { type: 'store/replace' }));
        }
    };
}
    ...
    return (state, action) => {
        switch (action.type) {
            ...
            case 'playlists/upsertMany': {
                const incoming = Array.isArray(action.payload) ? action.payload : [];
                const map = new Map((state.playlists || []).map((playlist) => [playlist.id, playlist]));
            ...
            case 'automation/setRules': {
                ...
                return { ...state, automationRules: rules.slice().sort((a, b) => (a.priority || 0) - (b.priority || 0)) };
            }
            case 'queue/set': {
